...
class ImageDataGenerator(object):
    ...
    def process(self, x, rng=np.random.RandomState(seed=SEED)):
        # get next sync_seed
...
base_dir = '/home/ubuntu/open sprayer/shuffled_plantnet/'
...
early_stop = keras.callbacks.EarlyStopping(monitor='val_loss', patience=8, verbose=1)                                             
...
train_datagen.set_pipeline([random_crop, random_transform, standardize, contrast_stretch])
# train_datagen = ImageDataGenerator(
#                     rescale = 1./255,
#                     rotation_range = 60,# using a directory with pre-augmented images + originals
#                     width_shift_range = 0.4,
...
history = model.fit_generator(
                train_batches,
                steps_per_epoch=(21250//32+1),
                epochs=50,
                validation_data=test_batches,
                #validation_split=0.3,#train validation split
        #        class_weight = {0:4, 1:3, 2:3, 3:3},
                validation_steps=(3430//32+1),
                verbose=1,
...
preds = model.predict_generator(test_batches, steps=(3430//32+1), verbose=1)
# #test_preds = np.round(preds).reshape(-1)
filenames = np.array(test_batches.filenames)
test_truth = test_batches.classes

predictions = pd.Series([np.argmax(preds[i]) for i in range(3430)])
actuals = pd.Series(test_truth)
print(classification_report(actuals, predictions, labels=[0,1,2,3], target_names=['forbs', 'grass', 'ground', 'weeds']))

import pickle
with open('weeds_{}_preds.pkl'.format(RUN), 'wb') as picklefile:
    pickle.dump(predictions, picklefile)
    
with open('weeds_{}_truth.pkl'.format(RUN), 'wb') as picklefile:
    pickle.dump(test_truth, picklefile)
    
with open('weeds_{}_filenames.pkl'.format(RUN), 'wb') as picklefile:
